...
def extractFace():
    ...
    user_image.data_original_as_base64 = request_param['data_orginal_as_base64']
    ...
    return responce_as_json('OK', 'sucess', 
    user_image.genre,
    user_image.genre_percentage_detail,
     ui_paths)

def responce_as_json(status, message, genre = None,genre_percentage_detail = None, images = None):
    return jsonify({
        'status':status,
        'message':message,
        'genre':genre,
        'genre_percentage_detail':genre_percentage_detail,
        'images':images
        })
