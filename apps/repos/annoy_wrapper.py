...
from util.JGSUtil import Dumper,resource_folder
...
class AnnoyWrapper:
    ...
    def get_ui_list(self, genre, feature_Value):
        ...
        index_list = annoy_db.get_nns_by_vector(
            self.__convert_vector(feature_Value), 
            10, search_k=-1, include_distances=False)

        ui_paths = [images[i].original_path for i in index_list]
        return ui_paths

    def init(self):
        ROOT = resource_folder("images")
    ...
    def load_annoy(self):
        ...
        for key in self.__images_dic.keys():
            annoy_file_path = resource_folder(f'annoy/{key}.ann')
            ...
            for i, image in enumerate(self.__images_dic[key]):
                if not image.exist_file:
                    print(f"nothing file >> {image.original_path}")
                    continue
                annoy_db.add_item(i, self.__convert_vector(image.feature))
            annoy_db.build(10)
            annoy_db.save(annoy_file_path)

    def __convert_vector(self, feature):
    ...
    def __create_images(self, d):
        ...
        for image_name in file_paths:
            ...
            img.original_path = os.path.join(original_path_root, image_name)
